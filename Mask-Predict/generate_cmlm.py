...
def main(args):
    ...
    has_target = True
    timer = TimeMeter()
    if args.context_dir == 'both':
        total_context = args.len_context * 2 + 1
    elif args.context_dir == 'right':
        total_context = args.len_context + 1
    if 'rnn' not in args.decoding_strategy:
        total_context = args.len_context * 2 + 1
    windows = []
    scores = []
    with progress_bar.build_progress_bar(args, itr) as t:

        translations = generate_batched_itr(t, strategy, models, tgt_dict, total_context, length_beam_size=args.length_beam, use_gold_target_len=args.gold_target_len)
        for sample_id, src_tokens, target_tokens, hypos, pred_len in translations:
            ...
            if align_dict is not None:
                ...
                if has_target:
                    target_str = dict.string(target_tokens, None, escape_unk=True,
                                                remove_eos=False)
                    if args.dehyphenate:
                        target_str = dehyphenate(target_str)
            # with open(args.translation_output_file, 'a') as f:
            #     ori_len = len(target_str.split()) // total_context
            #     if args.context_dir in ['both', 'left']:
            #         start_index = args.len_context
            #     elif args.context_dir == 'right':
            #         start_index = 0
            #     if 'rnn' not in args.decoding_strategy:
            #         start_index = 0
                
            #     f.write('ref: ' + ' '.join(target_str.split()[0::total_context]) + '\n')
            #     for seq in tgt_seq[1:]:
            #         seq = seq.split()
            #         for i in range(0, total_context * pred_len.item(), total_context):
            #             f.write('hyp: ' + ' '.join(seq[i:i+total_context]) + '\n')
            # for seq in tgt_seq:
                # seq = seq.split()
                # seq = np.reshape(seq[:total_context * pred_len.item()], (-1, total_context)).tolist()
                # for i in range(0, total_context * pred_len.item(), total_context):
                # windows.append(seq)
            # for score in token_probs:
            #     scores.append(score)
            ...
            if not args.quiet:
                ...
                if has_target:
                    target_str = dict.string(target_tokens[0::total_context], args.remove_bpe, escape_unk=True)
                    ...
                    num_sentences += 1
        # with open(args.translation_output_file, 'w') as f:
        #     for win, score in zip(windows, scores) :
        #         win_dict = {'windows': win, 'scores': score}

        #         json_win_dict = json.dumps(win_dict)
        #         f.write(json_win_dict + '\n')
...
def generate_batched_itr(data_itr, strategy, models, tgt_dict, total_context, length_beam_size=None, use_gold_target_len=False, cuda=True):
    ...
    for sample in data_itr:
        ...
        with torch.no_grad():
            ...
            hypos, pred_len = generate(strategy, encoder_input, models, tgt_dict, length_beam_size, gold_target_len)
            # len_iter = len(tgt_tokens_list)
            batch = s['target'].size(0)
            # tgt_tokens_list = np.reshape(tgt_tokens_list, (batch, len_iter, -1, total_context)).tolist()
            # token_probs_list = np.reshape(token_probs_list, (batch, len_iter, -1, total_context)).tolist()
            for batch in range(hypos.size(0)):
                ...
                # tgt_seq = tgt_tokens_list[batch]
                # token_probs = token_probs_list[batch]
                # tgt_seq = [tgt_token_single_list[batch] for tgt_token_single_list in  tgt_tokens_list]
                yield example_id, src, ref, hypo, pred_len[batch]
...
def generate(strategy, encoder_input, models, tgt_dict, length_beam_size, gold_target_len):
    ...
    # hypotheses = hypotheses.view(bsz, length_beam_size, -1)
