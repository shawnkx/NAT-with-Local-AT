...
@register_model('bert_transformer_seq2seq_mlp')
...
class SelfTransformerDecoder(FairseqIncrementalDecoder):
    ...
    def __init__(self, args, dictionary, embed_tokens, embed_scale=None, no_encoder_attn=False, left_pad=False,
        ...
        self.left_context_project = nn.ModuleList([])
        self.left_context_project.extend([
            nn.Linear(2 * self.embed_dim, self.embed_dim, bias=False)
            for _ in range(self.len_context)])
        self.right_context_project = nn.ModuleList([])
        self.right_context_project.extend([
            nn.Linear(2 * self.embed_dim, self.embed_dim, bias=False)
            for _ in range(self.len_context)])
        # self.center_project = nn.Linear(self.embed_dim, self.embed_dim, bias=False)
    ...
    def forward(self, prev_output_tokens, encoder_out=None, incremental_state=None):
        ...
        # T x B x C -> B x T x C
        x = x.transpose(0, 1)
        # x = gelu(self.center_project(x))
        # x = F.dropout(x, p=self.relu_dropout, training=self.training)
        if self.adaptive_softmax is None and self.load_softmax:
            # project back to size of vocabulary
            if self.share_input_output_embed:
                cur_x = F.linear(x, self.embed_tokens.weight)
            else:
                cur_x = F.linear(x, self.embed_out)
        x_max = torch.argmax(cur_x, dim=-1)
        x_max_emb = self.embed_tokens(x_max)
        x_max_emb = torch.cat((x_max_emb, x), dim=-1)
        left_context_x = []
        for linear_layer in self.left_context_project:
            temp_left = gelu(linear_layer(x_max_emb))
            temp_left = F.dropout(temp_left, p=self.relu_dropout, training=self.training)
            left_context_x.append(temp_left)
        right_context_x = []
        for linear_layer in self.right_context_project:
            temp_right = gelu(linear_layer(x_max_emb))
            temp_right = F.dropout(temp_right, p=self.relu_dropout, training=self.training)
            right_context_x.append(temp_right)
        ...
        if self.adaptive_softmax is None and self.load_softmax:
            # project back to size of vocabulary
            if self.share_input_output_embed:
                temp_left = torch.cat([F.linear(x, self.embed_tokens.weight) for x in left_context_x], dim=1)
                temp_right = torch.cat([F.linear(x, self.embed_tokens.weight) for x in right_context_x], dim=1)
                temp_x = torch.cat((temp_left, cur_x, temp_right), dim=1)
                temp_x = temp_x.unsqueeze(1).view(x.size(0),self.len_context * 2 + 1, -1, temp_x.size(2))
                output_context_x = temp_x.transpose(1, 2).contiguous().view(x.size(0), -1, temp_x.size(3))
                # temp_x = torch.cat([F.linear(x, self.embed_out) for x in context_x], dim=1)
                # for x in range(context_x):
                #     output_context_x.append(F.linear(x, self.embed_tokens.weight))
        ...
        return output_context_x, {'attn': attn, 'inner_states': inner_states, 'predicted_lengths': encoder_out['predicted_lengths']}
...
@register_model_architecture('bert_transformer_seq2seq_mlp', 'bert_transformer_seq2seq_mlp')
...
@register_model_architecture('bert_transformer_seq2seq_mlp', 'bert_transformer_seq2seq_mlp_big')
