...
@register_model('bert_transformer_seq2seq_rnn')
...
class SelfTransformerDecoder(FairseqIncrementalDecoder):
    ...
    def __init__(self, args, dictionary, embed_tokens, embed_scale=None, no_encoder_attn=False, left_pad=False,
        ...
        self.len_context = args.len_context
        self.context_dir = args.context_dir
        ...
        self.lstm_hidden_project = nn.Linear(self.embed_dim, self.embed_dim, bias=False)
        self.lstm_cell_project = nn.Linear(self.embed_dim, self.embed_dim, bias=False)
        self.context_project = nn.LSTMCell(self.embed_dim, self.embed_dim)
    ...
    def forward(self, prev_output_tokens, ctx_src=None, encoder_out=None, incremental_state=None):
        """
        Args:
        ...
        # embed position
        # ctx_src_pos_emb = self.embed_positions(
        #     ctx_src,
        # ) if self.embed_positions is not None else None
        ctx_src_pos_emb = None
        ctx_src_emb = self.embed_tokens(ctx_src)
        ...
        if positions is not None:
            x += positions
        if ctx_src_pos_emb is not None:
            ctx_src_emb += ctx_src_pos_emb
        ctx_src_emb = F.dropout(ctx_src_emb, p=self.dropout, training=self.training)
        if self.context_dir == 'both':
            ctx_src_emb = ctx_src_emb.view(-1, self.len_context * 2 + 1, ctx_src_emb.size(2))
        elif self.context_dir =='right':
            ctx_src_emb = ctx_src_emb.view(-1, self.len_context + 1, ctx_src_emb.size(2))
        ...
        ctx_src_emb = ctx_src_emb.transpose(0, 1)
        GRU_output = None
        hidden = x.reshape(-1, x.size(2))
        cell = self.lstm_cell_project(hidden)
        hidden = self.lstm_hidden_project(hidden)
        for i in range(ctx_src_emb.size(0)):
            hidden, cell = self.context_project(ctx_src_emb[i], (hidden, cell))
            if GRU_output is None:
                GRU_output = hidden.unsqueeze(1)
            else:
                GRU_output = torch.cat((GRU_output, hidden.unsqueeze(1)), dim=1)
        # x, _ = self.context_project(ctx_src_emb, x)
        x = GRU_output
    ...
    def generate(self, prev_output_tokens, encoder_out=None, incremental_state=None):
        """
        Args:
            prev_output_tokens (LongTensor): previous decoder outputs of shape
                `(batch, tgt_len)`, for input feeding/teacher forcing
        ...
        decoder_padding_mask = prev_output_tokens.eq(self.padding_idx)
        ctx_src = prev_output_tokens.new_ones((torch.numel(prev_output_tokens))) * self.dictionary.boseg()
        ...
        pred_output = None
        if self.context_dir == 'both':
            total_context = self.len_context * 2 + 1
        elif self.context_dir == 'right':
            total_context = self.len_context + 1

        for _ in range(total_context):
            hidden, cell = self.context_project(ctx_src_emb, (hidden, cell))
            if self.adaptive_softmax is None and self.load_softmax:
                # project back to size of vocabulary
                if self.share_input_output_embed:
                    x = F.linear(hidden, self.embed_tokens.weight)
                else:
                    x = F.linear(hidden, self.embed_out)
            ctx_src = torch.argmax(x, dim=-1)
            ...
            if pred_output is None:
                pred_output = x.unsqueeze(1)
            else:
                pred_output = torch.cat((pred_output, x.unsqueeze(1)), dim=1)
            # pred_output = x if pred_output is None else torch.cat((pred_output, x), dim=1)
        # print(pred_output.shape, prev_output_tokens.size())
        # pred_output = pred_output.reshape(-1, pred_output.size(2))
        pred_output = pred_output.view(prev_output_tokens.size(0), -1, pred_output.size(2))
        return pred_output, {'attn': attn, 'inner_states': inner_states, 'predicted_lengths': encoder_out['predicted_lengths']}
...
class TransformerEncoder(FairseqEncoder):
    ...
    def forward(self, src_tokens, src_lengths, ctx_src=None):
        """
        Args:
...
@register_model_architecture('bert_transformer_seq2seq_rnn', 'bert_transformer_seq2seq_rnn')
...
@register_model_architecture('bert_transformer_seq2seq_rnn', 'bert_transformer_seq2seq_rnn_big')
